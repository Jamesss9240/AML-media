    ...
    (error, response, body) => {
      ...
      request(
        ...
        (error, response, body) => {
          ...
          const isAlreadyBorrowed = userDoc.media_ids.some(
            (item) => item[0] === mediaId
          );
          if (isAlreadyBorrowed)
            return res
              .status(400)
              .send({ success: false, error: "Already borrowed" });
          ...
          if (parseInt(mediaDoc.quantity) > 0) {
            ...
            res
              .status(400)
              .send({ success: false, error: "No media available" });
          }
        }
      );
    }
