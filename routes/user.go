...
func isUserValid(email, password string) bool {
	...
	// log.Info("isUserValid", zap.String("email", email), zap.String("password", password))
...
func registerNewUser(fname, lname, email, password string) (*db.User, error) {
...
func updatePersonalDetails(c *gin.Context) {
	...
	// 	err = db.UpdateMember(db.Member{FirstName: fname, LastName: lname, Email: email, Password: password, DateOfBirth: dob})
	// 	if err != nil {
	// 		fmt.Println(err)
	// 		c.HTML(http.StatusBadRequest, "personal-details.html", gin.H{
	// 			"ErrorTitle":   "Update Failed",
	...
	// } else {
	c.HTML(http.StatusBadRequest, "personal-details.html", gin.H{
		"ErrorTitle":   "Update Failed because feature is not implemented",
		"ErrorMessage": "Will be done"})
	// return
	// }
}
